...
class OPLParty : public Party {
 private:
  /**
   * @brief Calculates the total number of votes received by all candidates in
   * the party.
   * @details
 ...
 public:
   ...
   * @param candidates A vector of pointers to `OPLCandidate` objects
  ...
  /**
   * @brief Assigns winning seats to candidates based on the number of seats won
   * by the party.
   * @param seats The number of seats won by the party to be assigned to
   * candidates.
   ...
   */
  void assign_seat_winners(int seats) override;
