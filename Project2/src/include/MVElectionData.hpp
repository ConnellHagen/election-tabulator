...
/**
 * @class MVElectionData
 * @brief Represents An (MV) Election.
 *
 ...
 */
class MVElectionData : public ElectionData {
 protected:
  /**
   * @brief Calculates the winning candidates for all seats to be won.
   * @param None.
   * @details
   * Sets the `winner` variable to true for all aggregated `MVCandidate`s who
   * won a seat.
   */
  void calculate_winners() override;

  /**
   * @brief Adds a MV-specific header to the audit log.
   * @param None.
   * @details
 ...
 public:
  /**
   * @brief Constructor for `MVElectionData`.
   ...
   */
  MVElectionData(std::vector<Party*> parties, int total_seats,
                  int total_ballots);

  /**
   * @brief Destructor for `MVElectionData`.
   */
  ~MVElectionData() override;
