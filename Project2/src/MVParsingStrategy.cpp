...
MVParsingStrategy::MVParsingStrategy()
{}

MVParsingStrategy::~MVParsingStrategy()
{}

ElectionData* MVParsingStrategy::create_election(std::vector<std::string> ballots) {
    ...
    auto MV_candidates = create_MV_candidates(file, num_candidates);
    ...
    for (const auto& tuple : MV_candidates) {
      Candidate* candidate =
    ...
    std::vector<Party*> MV_parties = create_MV_parties(MV_candidates);

    for (const auto& party : MV_parties) {
        num_ballots += party->get_total_votes();
    }

    // return nullptr;
    ElectionData* MV_election_data =
        new MVElectionData(MV_parties, num_seats, num_ballots);
    return MV_election_data;

}

std::vector<std::tuple<std::string, Candidate*>> MVParsingStrategy::create_MV_candidates(std::ifstream& file, int numCands) {
    ...
    if (std::getline(file, line)) {
        ...
        while (std::regex_search(search_start, line.cend(), matches, entry_regex)) {
            ...
            Candidate* candidate = new MVCandidate(name);
...
void MVParsingStrategy::assign_votes_to_candidates(std::ifstream& file, std::vector<Candidate*> candidates) {
    std::string line;
        ...
        for (size_t i = 0; i < tokens.size(); ++i) {
            ...
            if (!tokens.at(i).empty() && tokens.at(i) == "1") {
                ...
                if (i < candidates.size()) {
                    candidates.at(i)->set_num_votes(candidates.at(i)->get_num_votes() + 1);
                }
            }
        }
    }
...
std::vector<Party*> MVParsingStrategy::create_MV_parties(std::vector<std::tuple<std::string, Candidate*>> candidates) {
    std::vector<Party*> parties;

    // TeMVrary storage for party names and candidates
    ...
    for (size_t i = 0; i < party_names.size(); ++i) {
        Party* party = new MVParty(partyCandidates.at(i), party_names.at(i));
